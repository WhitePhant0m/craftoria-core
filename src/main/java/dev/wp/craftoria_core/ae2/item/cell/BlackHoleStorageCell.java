...
public class BlackHoleStorageCell implements StorageCell {
    ...
    public BlackHoleStorageCell(ItemStack stack, ISaveProvider container) {
        ...
        var storage = stack.get(AE2Components.SINGULARITY_STORAGE);
    ...
    public void persist() {
        ...
        if (storedItem == null || count.signum() < 1) {
            stack.remove(AE2Components.SINGULARITY_STORAGE);
        } else {
            var storage = new BlackHoleStorage(storedItem, count);
            stack.set(AE2Components.SINGULARITY_STORAGE, storage);
        }
